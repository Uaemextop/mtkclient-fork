...
echo.
:: Auto-create adb_usb.ini with all supported VIDs
:: (Replaces the manual ADB_add_VID.bat from MTK SP Drivers 20160804)
echo       Configuring ADB VIDs in adb_usb.ini...
if not exist "%USERPROFILE%\.android" mkdir "%USERPROFILE%\.android"
set "ADB_INI=%USERPROFILE%\.android\adb_usb.ini"
:: Write all VIDs (overwrite to avoid duplicates)
(
echo # mtkclient USB VIDs for ADB device detection
echo # Auto-generated by install_drivers.bat
echo 0x0E8D
echo 0x18D1
echo 0x0FCE
echo 0x1004
...
echo       Created %ADB_INI% with MTK/Google/Sony/LG/OPPO VIDs.
echo.
