    ...
    )
{
    ...
    /*
     * If the device was surprise-removed (e.g. mtkclient USB reset
     * during speed upgrade: close(reset=True)), don't try to reset
     * the pipe — the device is gone. Return FALSE to stop the reader.
     *
     * For transient errors (timeout, stall), return TRUE to reset
     * the pipe and restart the continuous reader.
     */
    ...
    )
{
    ...
    /* Don't restart if device was removed (USB reset / disconnect) */
    if (Status == STATUS_NO_SUCH_DEVICE ||
        Status == STATUS_DEVICE_NOT_CONNECTED ||
        Status == STATUS_DEVICE_REMOVED) {
        return FALSE;
