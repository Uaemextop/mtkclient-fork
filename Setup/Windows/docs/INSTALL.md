...
If you downloaded the installer package produced by this repository's Windows driver workflow:
...
certutil -addstore Root mtk_usb2ser.cer
certutil -addstore TrustedPublisher mtk_usb2ser.cer
pnputil /add-driver Setup\Windows\driver\opensource\mtk_usb2ser.inf /install
```
...
7. Navigate to the `Setup\Windows\driver\opensource\` folder (or the workflow build output folder)
...
Or install via Device Manager by browsing to the `Setup\Windows\driver\CDC\` folder.
...
```powershell
# Run as Administrator
powershell -ExecutionPolicy Bypass -File Setup\Windows\installer\uninstall_driver.ps1
```
